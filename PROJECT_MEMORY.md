...
For the GitHub Pages deployment, both Angular handlers now post JSON to FormSubmit's cross-origin AJAX endpoint for `nabilmaruf1122@gmail.com`, with separate newsletter/contact subjects and the existing honeypot forwarded as `_honey`. FormSubmit requires the recipient to confirm its first activation email before it forwards submissions.
